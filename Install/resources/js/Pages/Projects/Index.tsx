...
function ProjectCard({
    ...
    const aiBuilderUrl = `/project/${project.id}`;
    ...
    return (
        <div className="group relative">
            <Link href={isTrash ? '#' : projectUrl} className={isTrash ? 'pointer-events-none' : ''}>
            ...
            <div>
                <Link
                    href={isTrash ? '#' : projectUrl}
                    className={`font-medium truncate group-hover:text-primary transition-colors block ${isTrash ? 'pointer-events-none' : ''}`}
                >
                    {project.name}
                </Link>
                    ...
                    <SidebarInset className="admin-layout__inset">
                        <div className="admin-layout__body">
                            ...
                            <main className="admin-layout__main">
                                <div className="admin-layout__content-shell admin-layout__content-surface admin-layout__content-surface--page">
                                {isLoading ? (
                                    <ProjectsSkeleton />
                                ) : (
                                    <>
                                    ...
                                    <div className={`grid ${getGridClasses()} gap-6`}>
                                        {/* Create new project card - only when not in trash */}
                                        {activeTab !== 'trash' && (
                                            <Link
                                                href="/create"
                                                className="group relative flex flex-col aspect-[4/3] rounded-xl border-2 border-dashed border-muted-foreground/30 bg-muted/30 hover:border-primary hover:bg-muted/50 transition-colors overflow-hidden"
                                            >
                                                <div className="flex-1 flex items-center justify-center">
                                                    <Plus className="h-12 w-12 text-muted-foreground group-hover:text-primary transition-colors" />
                                                </div>
                                                <div className="p-3 border-t border-muted-foreground/20">
