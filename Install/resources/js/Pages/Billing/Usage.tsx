...
export default function Usage({
    ...
    const handleBuyCredits = async (pack: CreditPack) => {
        ...
        setIsPurchasingPack(pack.key);

        try {
            const response = await axios.post(route('billing.usage.purchase'), {
                pack_key: pack.key,
            });

            setPurchaseMessage(response.data?.message ?? t('Credit top-up completed successfully.'));
            router.reload({
                only: ['stats', 'metering', 'credit_packs', 'referral_credit_balance'],
            });
        } catch (error) {
            if (axios.isAxiosError(error)) {
                setPurchaseError(error.response?.data?.error || t('Unable to complete credit top-up.'));
            } else {
                setPurchaseError(t('Unable to complete credit top-up.'));
            }
        } finally {
            ...
            <div className="space-y-6">
                ...
                <div className="grid gap-6 md:grid-cols-3">
                    ...
                    <Card>
                        <CardHeader className="pb-2">
                            ...
                            <CardDescription>{t('generate/edit/rebuild + orders/bookings')}</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-3 text-sm">
                            <div className="flex items-center justify-between">
                                <span className="text-muted-foreground">{t('AI Generate')}</span>
                                <span className="font-medium">{metering.ai_operations.generate.toLocaleString()}</span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-muted-foreground">{t('AI Edit')}</span>
                                <span className="font-medium">{metering.ai_operations.edit.toLocaleString()}</span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-muted-foreground">{t('AI Rebuild')}</span>
                                <span className="font-medium">{metering.ai_operations.rebuild.toLocaleString()}</span>
                            </div>
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <span className="font-medium">
                                    {metering.commerce.orders.toLocaleString()}
                                    {' / '}
                                    {metering.commerce.orders_limit === null
                                        ? t('Unlimited')
                                        : metering.commerce.orders_limit.toLocaleString()}
                                </span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-muted-foreground flex items-center gap-1">
                                ...
                                <span className="font-medium">
                                    {metering.booking.bookings.toLocaleString()}
                                    {' / '}
                                    {metering.booking.bookings_limit === null
                                        ? t('Unlimited')
                                        : metering.booking.bookings_limit.toLocaleString()}
                                </span>
                            </div>
                        </CardContent>
                    </Card>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            {t('Purchase overage packs using referral credits.')}
                        </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="flex items-center justify-between rounded-md border bg-muted/40 px-3 py-2 text-sm">
                            <span className="text-muted-foreground">{t('Referral Balance')}</span>
                            <span className="font-medium">${referral_credit_balance.toFixed(2)}</span>
                        </div>
                        ...
                        <div className="grid gap-3 md:grid-cols-3">
                            {credit_packs.map((pack) => {
                                const canAfford = referral_credit_balance >= pack.price;
                                const disabled = Boolean(isPurchasingPack) || !canAfford || stats.is_unlimited;

                                return (
                                    <div key={pack.key} className="rounded-lg border p-3">
                                        ...
                                        >
                                            {isPurchasingPack === pack.key ? t('Processing...') : t('Buy')}
                                        </Button>
                                    </div>
                                );
                            })}
                        ...
                        {stats.is_unlimited && (
                            <p className="text-sm text-muted-foreground">
                                {t('Your current plan is unlimited, so extra credit packs are not required.')}
                            </p>
                        )}
                    </CardContent>
                </Card>
