...
function SortablePlanCard({ plan, onDelete, onToggleStatus, renderFeatures, t, locale }: SortablePlanCardProps) {
    ...
    return (
        <div ref={setNodeRef} style={style} className="h-full">
            <Card
                className={`h-full flex flex-col min-h-[320px] ${!plan.is_active ? 'opacity-60' : ''} ${plan.is_popular ? 'ring-2 ring-primary' : ''} ${isDragging ? 'shadow-lg' : ''}`}
            >
                ...
                <CardContent className="flex-1 min-h-0 flex flex-col">
                    <ul className="space-y-2 overflow-y-auto max-h-[200px] pr-1">{renderFeatures(plan)}</ul>
                    <div className="mt-4 pt-4 border-t space-y-1 shrink-0">
                ...
                <CardFooter className="flex gap-2 shrink-0 flex-wrap">
...
export default function Index({ auth, plans }: PlansPageProps) {
    ...
    const handleDelete = (plan: Plan) => {
        ...
        if (confirm(t('Are you sure you want to delete this plan?'))) {
            router.delete(route('admin.plans.destroy', plan.id), {
                onSuccess: () => {
                    toast.success(t('Plan deleted'));
                    setPlanItems((items) => items.filter((item) => item.id !== plan.id));
                },
                onError: (errors) => {
