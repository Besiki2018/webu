...
function translateDynamicValue(value: unknown, t: (key: string, replacements?: Record<string, string | number>) => string): unknown {
    if (typeof value === 'string') {
        return t(value);
    }

    if (Array.isArray(value)) {
        return value.map((item) => translateDynamicValue(item, t));
    }
    ...
    if (value && typeof value === 'object') {
        return Object.fromEntries(
            Object.entries(value as Record<string, unknown>).map(([key, nestedValue]) => [
                key,
                translateDynamicValue(nestedValue, t),
            ])
        );
    }
...
export default function Landing({
    ...
    const localizedSections = useMemo(
        () => sections.map((section) => ({
            ...section,
            content: translateDynamicValue(section.content, t) as Record<string, string | string[] | null>,
            items: translateDynamicValue(section.items, t) as Array<Record<string, unknown>>,
        })),
        [sections, t]
    );
    ...
    const heroSection = useMemo(
        () => localizedSections.find((s) => s.type === 'hero'),
        [localizedSections]
    );
    const socialProofSection = useMemo(
        () => localizedSections.find((s) => s.type === 'social_proof'),
        [localizedSections]
    );
    const pricingSection = useMemo(
        () => localizedSections.find((s) => s.type === 'pricing'),
        [localizedSections]
    );
    const ctaSection = useMemo(
        () => localizedSections.find((s) => s.type === 'cta'),
        [localizedSections]
    );
    ...
    const enabledSections = useMemo(
        () => localizedSections.filter((s) => s.is_enabled && s.type !== 'trusted_by'),
        [localizedSections]
    );
            ...
            case 'hero':
                return (
                    <HeroSection
                        ...
                        initialHeadline={heroHeadlines[0] || headline || 'What will you build?'}
                        initialSubtitle={heroSubtitles[0] || subtitle || t('Create stunning websites by chatting with AI.')}
            ...
            default: {
                ...
                if (!Component) {
                    return (
                        <AnimatedSection key={section.type} delay={index * 50}>
                            <div
                                className="rounded-lg border border-amber-500/50 bg-amber-50 dark:bg-amber-950/20 p-6 text-center"
                                data-missing-section-type={section.type}
                            >
                                <p className="text-sm font-medium text-amber-800 dark:text-amber-200">
                                    Missing section: {section.type}
                                </p>
                                <p className="mt-1 text-xs text-muted-foreground">
                                    Add this component type to SECTION_COMPONENTS or remove the section.
                                </p>
                            </div>
                        </AnimatedSection>
                    );
    ...
    return (
        <>
            ...
            <div className="landing-font-scope">
