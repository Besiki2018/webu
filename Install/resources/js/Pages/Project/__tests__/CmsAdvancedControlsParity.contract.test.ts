...
describe('CMS advanced controls parity contracts', () => {
    it('defines canonical advanced control schema for custom css, attributes, visibility, and positioning', () => {
    ...
    it('applies advanced visibility, attributes, positioning and custom css presence markers in builder preview', () => {
        const cms = read(cmsPagePath);
    ...
    it('keeps runtime advanced controls handling aligned with builder preview contracts', () => {
        const service = read(templateImportServicePath);

        expect(service).toContain('var parseOptionalIntProp = function (value) {');
        expect(service).toContain('var advancedVisibilityProps = advancedProps.visibility && typeof advancedProps.visibility === \'object\' && !Array.isArray(advancedProps.visibility)');
        expect(service).toContain('var advancedPositioningProps = advancedProps.positioning && typeof advancedProps.positioning === \'object\' && !Array.isArray(advancedProps.positioning)');
        expect(service).toContain('var advancedAttributesProps = advancedProps.attributes && typeof advancedProps.attributes === \'object\' && !Array.isArray(advancedProps.attributes)');
