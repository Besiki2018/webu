...
export default function AiSettingsTab({
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        ultra_cheap_mode: settings?.ultra_cheap_mode !== false,
        openai_api_key: '',
        anthropic_api_key: '',
        grok_api_key: '',
        deepseek_api_key: '',
    ...
    if (!canUseOwnKey) {
        return (
            <div className="space-y-6">
                {/* Website generation: Ultra Cheap — shown to everyone */}
                <div className="bg-card p-4 shadow-sm border border-border rounded-lg sm:p-8">
                    <section>
                        <header>
                            <h2 className="text-lg font-medium text-foreground">
                                {t('Website generation')}
                            </h2>
                            <p className="mt-1 text-sm text-muted-foreground">
                                {t('Ultra Cheap Mode uses templates and presets (near-zero tokens). Pro AI uses the main model for custom content and layout.')}
                            </p>
                        </header>
                        <div className="mt-6 flex items-center justify-between rounded-lg border border-border p-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="font-medium">{t('Ultra Cheap Mode')}</p>
                                    <p className="text-sm text-muted-foreground">
                                        {t('Templates + rules, minimal AI. Default ON.')}
                                    </p>
                                </div>
                            </div>
                            <Switch
                                checked={data.ultra_cheap_mode}
                                onCheckedChange={(checked) => setData('ultra_cheap_mode', checked)}
                            />
                        </div>
                        <div className="mt-3">
                            <Button
                                ...
                                size="sm"
                                disabled={processing}
                                onClick={() => put('/profile/ai-settings', {
                                    onSuccess: () => toast.success(t('AI settings updated successfully')),
    ...
    return (
        <div className="space-y-6">
            {/* Website generation mode: Ultra Cheap (default) vs Pro AI */}
            <div className="bg-card p-4 shadow-sm border border-border rounded-lg sm:p-8">
                <section>
                    <header>
                        <h2 className="text-lg font-medium text-foreground">
                    ...
                    <p className="mt-2 text-xs text-muted-foreground">
                        {t('When OFF (Pro AI), website generation may use more credits for custom copy and layout.')}
                    </p>
                </section>
            </div>
                                        ...
                                        <div className="flex gap-2">
                                            <div className="relative flex-1">
                                                <Input
                                                    ...
                                                    value={(typeof data[getApiKeyField(provider) as keyof typeof data] === 'string' ? data[getApiKeyField(provider) as keyof typeof data] : '') as string}
                                                    onChange={(e) =>
                                            ...
                                            <Button
                                                ...
                                                onClick={() =>
                                                    handleTestConnection(
                                                        provider,
                                                        (typeof data[getApiKeyField(provider) as keyof typeof data] === 'string'
