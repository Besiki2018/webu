...
export default function ApplicationLogo({
    ...
    // Logo image sizes (kept compact when shown with text)
    ...
    // If logo exists, show image + site title when showText (same layout as fallback, smaller logo/title)
    if (logoUrl) {
        return (
            <div className={`flex items-center gap-2 ${className || ''}`} {...props}>
                <img
                    src={logoUrl}
                    alt={siteName}
                    className={`${imageSizeClasses[size]} object-contain shrink-0`}
                />
                {showText && (
                    <div className="min-w-0">
                        <span className={`font-semibold bg-gradient-to-r ${colors.textGradient} ${colors.textGradientDark} bg-clip-text text-transparent ${textSizeClasses[size]} block truncate`}>
                            {siteName}
                        </span>
                        {siteTagline && (size === 'lg' || size === 'md') && (
                            <p className="text-[10px] font-medium text-muted-foreground truncate">{siteTagline}</p>
                        )}
                    </div>
                )}
            </div>
