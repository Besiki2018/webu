...
export function ProductCardClassic({ product, basePath }: WebuProductCardProps) {
  const href = product.url.startsWith('http') ? product.url : path(basePath, product.url);
  return (
    <article className="webu-product-card webu-product-card--classic">
      ...
      <div className="webu-product-card__content">
        ...
        <div className="webu-product-card__price-line">
          <span className="webu-product-card__price">{product.price}</span>
          {product.old_price && (
            <span className="webu-product-card__price-old">{product.old_price}</span>
          )}
        </div>
      </div>
    </article>
