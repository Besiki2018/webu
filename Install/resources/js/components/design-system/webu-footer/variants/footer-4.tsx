...
/** Multi-column footer */
export function Footer4({ logo, logoUrl = '/', logoFallback = 'Store', menus = {}, contactAddress, copyright, basePath }: WebuFooterProps) {
  const menuKeys = Object.keys(menus);
  return (
    <footer className="webu-footer webu-footer--footer-4">
      <div className="webu-footer__inner">
        <div className="webu-footer__grid webu-footer__grid--multicol">
          <div className="webu-footer__brand">
            <Link href={path(basePath, logoUrl)} className="webu-footer__logo">{logo ?? logoFallback}</Link>
            {contactAddress && <p className="webu-footer__address">{contactAddress}</p>}
          </div>
          {menuKeys.map((key) => (
