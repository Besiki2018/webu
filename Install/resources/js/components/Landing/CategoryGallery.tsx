...
export function CategoryGallery({ content, items, settings: _settings }: CategoryGalleryProps = {}) {
    ...
    return (
        <section className="webu-category-gallery py-16 lg:py-20 border-t border-border/60">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                {/* Section Header */}
                <div className="text-center mb-12">
                    <h2 className="webu-category-gallery__title text-3xl md:text-4xl font-semibold tracking-tight mb-4">
                        {title}
                    </h2>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                    {categories.map((category) => {
                        ...
                        return (
                            <Card
                                key={category.name}
                                className="group cursor-pointer border border-border shadow-none hover:bg-accent/40 transition-colors"
                            >
