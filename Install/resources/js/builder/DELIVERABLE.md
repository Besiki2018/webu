...
These components are **fully migrated**: schema + defaults + central registry + canvas render from props + sidebar from schema + single update pipeline. They are the source of truth.
...
- In registry: yes (REGISTRY + central componentRegistry)
...
## 2. Legacy components (registry only, not central)

These section types are **in the main REGISTRY** and have normalized schema + defaults from `componentRegistry` (parameters + buildFoundationFields). They are **not** in the central registry; the canvas renders them via `Builder*CanvasSection` from `registryComponents.tsx`. They are isolated as “legacy” in the sense that they do not yet have a dedicated schema file + defaults file + real React component in the central registry. They remain usable and editable; future migration follows the same pattern as Header/Footer/Hero.
...
**Policy:** No conflicting parallel implementations. The main builder registry is the single source of truth. Legacy entries are not removed; they are excluded from the **central** registry until migrated. Sidebar and update pipeline still use the same schema (normalized) and the same update path.

---

## 3. Registry structure summary

- **Main registry** (`componentRegistry.ts`):
  ...
  - `getComponentRuntimeEntry(registryId)` returns `BuilderComponentRuntimeEntry`: componentKey, component, schema, defaults. Component is either from central registry or a Builder*CanvasSection.

- **Central registry** (`centralComponentRegistry.ts`):
  ...
  - Only migrated components (Header, Footer, Hero) are in the central registry. Canvas uses them for full-fidelity rendering; all others use runtime entry from main REGISTRY + resolveCanvasComponent.

- **Lookup flow:** Section type → getComponentRuntimeEntry(type) → if central has entry → use central component + mapBuilderProps; else → use CanvasComponent from resolveCanvasComponent with resolveComponentProps(type, props).
...
- **Component schema files (migrated):** Each migrated component has a `Component.schema.ts` that exports a schema compatible with BuilderComponentSchema (e.g. built from ComponentSchemaDef via conversion). Defaults in `Component.defaults.ts`; both wired into central registry.
...
- (No breaking changes to existing files; types.ts re-exports from componentRegistry and centralComponentRegistry. Existing imports remain valid.)
...
- `builder/centralComponentRegistry.ts` — componentRegistry, REGISTRY_ID_TO_KEY, getCentralRegistryEntry, isInCentralRegistry.
...
- `builder/visual/BuilderCanvas.tsx` — renderRegistrySection, central vs runtime component resolution.
...
- **None.** The base architecture is in place; Header, Footer, and Hero are fully migrated and validated. Legacy section types work through the same registry and update pipeline; they can be migrated one-by-one to the central registry when their real component + schema + defaults are added.
