...
describe('useBuilderChat', () => {
    ...
    it('includes responsive and variant context for selected element prompts', async () => {
        mockedAxios.post.mockResolvedValueOnce({
            data: { session_id: 'session-1', builder_id: 1 },
        });
        ...
        await act(async () => {
            await result.current.sendMessage('Make this heading smaller', {
                elementContext: {
                    tagName: 'h1',
                    ...
                    allowedUpdates: {
                        scope: 'element',
                        operationTypes: ['updateText', 'updateSection'],
                    ...
                    responsiveContext: {
                        ...
                        availableInteractionStates: ['normal', 'hover', 'focus', 'active'],
                        supportsVisibility: true,
                        supportsResponsiveOverrides: true,
                        visibleFieldPaths: ['title', 'title_typography', 'responsive.mobile.padding_top', 'states.hover.background_color'],
                        responsiveFieldPaths: ['responsive.hide_on_mobile', 'responsive.mobile.padding_top'],
        ...
        expect(mockedAxios.post).toHaveBeenCalledWith(
            ...
            expect.objectContaining({
                prompt: expect.stringContaining('Current Breakpoint: mobile'),
            }),
        );
        expect(mockedAxios.post).toHaveBeenCalledWith(
            '/builder/projects/1/start',
            expect.objectContaining({
                prompt: expect.stringContaining('Current Interaction State: hover'),
            }),
        );
        expect(mockedAxios.post).toHaveBeenCalledWith(
            '/builder/projects/1/start',
            expect.objectContaining({
                prompt: expect.stringContaining('Responsive Field Paths (mobile): responsive.hide_on_mobile, responsive.mobile.padding_top'),
            }),
        );
    });
    ...
    describe('safety fallback polling', () => {
        ...
        it('sets hasFileChanges when detecting completed status', async () => {
            ...
            // Advance timer to the fallback poll interval (6s) and flush async
            await act(async () => {
        ...
        it('detects completed status at 5 second interval', async () => {
            ...
            // At 6 seconds, the polling interval fires and detects 'completed'
            await act(async () => {
                vi.advanceTimersByTime(6000);
                await flushPromises();
            });
        ...
        /** Task 7: no rerender-induced immediate repoll — callback prop change must not trigger extra status GET */
        it('does not immediately repoll status when callback props change', async () => {
            mockedAxios.post.mockResolvedValueOnce({ data: { session_id: 'session-1' } });
            mockedAxios.get.mockResolvedValue({ data: { status: 'running', recent_history: [] } });

            const { result, rerender } = renderHook(
                ({ onComplete, onError, onBuildComplete }) => useBuilderChat(1, {
        ...
        it('429 backoff: does not repoll status immediately after 429', async () => {
            mockedAxios.post.mockResolvedValueOnce({ data: { session_id: 'session-1' } });
            const statusUrl = '/builder/projects/1/status';
            mockedAxios.get.mockImplementation((url: string) => {
                if (url?.includes(statusUrl)) {
                    const err = new Error('Too Many Requests') as Error & { response?: { status?: number; headers?: Record<string, string> } };
                    err.response = { status: 429, headers: { 'retry-after': '2' } };
                    return Promise.reject(err);
                }
                return Promise.resolve({ data: {} });
            });
            mockedAxios.isAxiosError = vi.fn((v: unknown) => (v as { response?: { status?: number } })?.response?.status === 429);
            ...
            const statusGetCount = () => mockedAxios.get.mock.calls.filter((c) => String(c[0]).includes(statusUrl)).length;
            await waitFor(() => expect(statusGetCount()).toBeGreaterThanOrEqual(1));
