...
describe('useChatSounds', () => {
    ...
    describe('error handling', () => {
        it('handles AudioContext creation failure gracefully', () => {
            const ThrowingAudioContextConstructor = vi.fn(function ThrowingAudioContextConstructor() {
                throw new Error('AudioContext not allowed');
            });
            (global as typeof globalThis & { AudioContext: typeof AudioContext }).AudioContext =
                ThrowingAudioContextConstructor as unknown as typeof AudioContext;
