...
export function useSessionReconnection(
    ...
    const statusCheckBackoffUntilRef = useRef(0);
    ...
    const checkSessionStatus = useCallback(async (): Promise<SessionStatus | null> => {
        ...
        if (Date.now() < statusCheckBackoffUntilRef.current) {
            return null;
        }
        ...
        try {
            ...
            if (axios.isAxiosError(error) && error.response?.status === 429) {
                ...
                statusCheckBackoffUntilRef.current = Date.now() + (
            ...
            statusCheckBackoffUntilRef.current = Date.now() + 5000;
            setSessionIsActive(false);
            return null;
        } finally {
