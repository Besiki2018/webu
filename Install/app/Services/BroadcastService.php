...
class BroadcastService
{
     ...
     * @return array{configured: bool, working: bool, error: string|null, driver: string}
     */
    ...
    protected function testReverbConnection(array $settings): array
    {
        ...
        if (empty($host) || empty($appId) || empty($key) || empty($secret)) {
            return [
                'success' => false,
                'error' => 'Reverb credentials are incomplete (host, app_id, key, secret required).',
            ];
        }
    ...
    public function getErrorMessage(): ?string
    {
        ...
        if (! $health['working']) {
            $driver = $health['driver'] ?? 'pusher';
            $hint = $driver === 'reverb'
                ? ' If using Reverb, start it with: php artisan reverb:start'
                : ' Check Pusher dashboard and credentials.';
            return 'Real-time server is not responding. Please check that your broadcast server (Pusher/Reverb) is running and credentials are correct.'.$hint;
        }
