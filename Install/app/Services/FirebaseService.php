...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    match /{$prefix}/{collection}/{document=**} {
      ...
      // Allow read/write access to authenticated users who own the project
      allow read, write: if request.auth != null
                         && request.auth.token.projectId == '{$projectId}';

      // Allow read/write access with valid project token
      allow read, write: if request.auth != null
                         && request.auth.token.type == 'project_token'
                         && request.auth.token.projectId == '{$projectId}';
    }

    // Deny access to other projects' data
    match /projects/{otherProject}/{collection}/{document=**} {
