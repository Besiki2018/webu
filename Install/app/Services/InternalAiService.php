...
class InternalAiService
{
    ...
    protected const DEFAULT_MODELS = [
        ...
        AiProvider::TYPE_CLAUDE => 'claude-sonnet-4-6',
    ...
    {
        $preferredProviderIds = $this->preferredProviderIds();

        foreach ($preferredProviderIds as $providerId) {
            $provider = $this->findActiveProviderById($providerId);
        ...
        return $this->findFirstActiveProvider();
    }
    ...
    protected function preferredProviderIds(): array
    {
        return array_values(array_filter([
            SystemSetting::get('internal_ai_provider_id'),
            SystemSetting::get('default_ai_provider_id'),
    ...
    /**
     * Get the configured model (default: cheaper models for suggestions, landing, etc.).
     */
    ...
    /**
     * Get the model for Webu project edit (code changes). Uses strongest model per provider
     * so code edits are accurate and the project succeeds.
     */
    public function getModelForProjectEdit(): string
    {
        $customModel = SystemSetting::get('internal_ai_project_edit_model');
        if (! empty($customModel)) {
            return $customModel;
        }
        ...
        $provider = $this->getProvider();
        if (! $provider) {
            $models = AiProvider::DEFAULT_MODELS[AiProvider::TYPE_OPENAI] ?? [];
            return is_array($models) && $models !== [] ? $models[0] : 'gpt-4.1';
        }
    ...
    /**
     * Completion for Webu project edit (code changes). Uses strongest model.
     */
    public function completeForProjectEdit(string $prompt, int $maxTokens = 8000): ?string
    {
        $provider = $this->getProvider();
        if (! $provider) {
            return null;
        }
        $model = $this->getModelForProjectEdit();

        return $this->callProviderWithMaxTokens($provider, $model, $prompt, $maxTokens);
    }
    ...
    public function getSuggestions(int $count = 4, ?string $locale = null): array
    {
        ...
        try {
            ...
            AiErrorAudit::report('Internal AI suggestions failed', ['locale' => $locale], $e);
        }
...
PROMPT;
    ...
    /**
     * Generic completion: send prompt to configured provider and return raw response text.
     * Use for one-off prompts (e.g. CMS patch proposal). Returns null if provider not configured or call fails.
     */
    public function complete(string $prompt, int $maxTokens = 2000): ?string
    {
        $provider = $this->getProvider();
        if (! $provider) {
            return null;
    ...
    /**
     * Generate a short, natural-language chat reply after a site edit (so the chat feels like talking to a person, not an auto-responder).
     * Reply is in the same language as the user's message.
     *
     * @param  array<int, string>  $actionSummary  e.g. ["Updated hero headline", "Added pricing section"]
     */
    public function generateSiteEditReply(string $userMessage, array $actionSummary, ?string $locale = null): ?string
    {
        if (! $this->isConfigured()) {
            return null;
        }
        ...
        $locale = $locale ?? app()->getLocale();
        $languageInstruction = $this->getLanguageInstruction($locale);
        $summaryLine = $actionSummary !== [] ? implode("\n", array_map(fn ($s) => '- '.$s, $actionSummary)) : 'Changes were applied to the page.';

        $prompt = <<<PROMPT
You are Webu, a developer assistant that edits and creates projects. The user asked for a change and the system has applied it.
...
Be direct and professional. In 1–2 sentences confirm what you changed, then clearly list the changes (e.g. "I've made the following changes:" and bullet points). Sound like a developer summarizing a commit, not a generic chatbot.
{$languageInstruction}

Reply with only the response text, no quotes or prefix.
PROMPT;

        $reply = $this->complete($prompt, 150);
    ...
    {
        return $this->callProviderWithMaxTokens($provider, $model, $prompt, 300);
    }

    /**
     * Call the appropriate AI provider API with configurable max tokens.
     */
    protected function callProviderWithMaxTokens(AiProvider $provider, string $model, string $prompt, int $maxTokens): ?string
    {
        return match ($provider->type) {
            AiProvider::TYPE_OPENAI,
            AiProvider::TYPE_GROK,
            AiProvider::TYPE_DEEPSEEK => $this->callOpenAiCompatibleWithTokens($provider, $model, $prompt, $maxTokens),

            AiProvider::TYPE_ANTHROPIC,
            AiProvider::TYPE_CLAUDE,
            AiProvider::TYPE_ZHIPU => $this->callAnthropicWithTokens($provider, $model, $prompt, $maxTokens),
    ...
    {
        return $this->callOpenAiCompatibleWithTokens($provider, $model, $prompt, 300);
    }

    /**
     * Call OpenAI-compatible API with configurable max tokens.
     */
    protected function callOpenAiCompatibleWithTokens(AiProvider $provider, string $model, string $prompt, int $maxTokens): ?string
    {
        $payload = [
            'model' => $model,
            'messages' => [
        ...
        ])->timeout(30)->post($provider->getBaseUrl().'/chat/completions', $payload);
        ...
        if (! $response->successful()) {
            ...
            AiErrorAudit::report('OpenAI-compatible internal AI call failed', [
                'provider' => $provider->type,
                'status' => $response->status(),
                'response_truncated' => substr($response->body(), 0, 500),
            ]);
    ...
    {
        return $this->callAnthropicWithTokens($provider, $model, $prompt, 300);
    }

    /**
     * Call Anthropic-compatible API with configurable max tokens.
     */
    protected function callAnthropicWithTokens(AiProvider $provider, string $model, string $prompt, int $maxTokens): ?string
    {
        ...
        $response = Http::withHeaders([
            ...
            'Content-Type' => 'application/json',
        ])->timeout(30)->post($baseUrl.'/messages', [
            'model' => $model,
        ...
        if (! $response->successful()) {
            ...
            AiErrorAudit::report('Anthropic-compatible internal AI call failed', [
                'provider' => $provider->type,
                'status' => $response->status(),
                'response_truncated' => substr($response->body(), 0, 500),
            ]);
     ...
     * Suggestions are in the given locale (site/app language).
     *
     * @param  array  $conversationHistory  Array of messages with 'role' and 'content'
     * @param  int  $count  Number of suggestions to generate
     * @param  string|null  $locale  Site/app locale (e.g. 'ka', 'en'). Suggestions returned in this language.
     * @return array|null Returns null if AI is not configured or fails
     */
    public function getChatSuggestions(array $conversationHistory, int $count = 3, ?string $locale = null): ?array
    {
        $locale = $locale ?? app()->getLocale();
        ...
        try {
            $suggestions = $this->generateChatSuggestions($conversationHistory, $count, $locale);
            if ($suggestions !== null && count($suggestions) > 0) {
                return $suggestions;
            }
        } catch (\Exception $e) {
            ...
            AiErrorAudit::report('Internal AI chat suggestions failed', [], $e);
        }
     ...
     */
    protected function generateChatSuggestions(array $conversationHistory, int $count, ?string $locale = null): ?array
    {
        $provider = $this->getProvider();
        $model = $this->getModel();

        $prompt = $this->buildChatSuggestionsPrompt($conversationHistory, $count, $locale);
     ...
     */
    protected function buildChatSuggestionsPrompt(array $conversationHistory, int $count, ?string $locale = null): string
    {
        $locale = $locale ?? app()->getLocale();
        $languageInstruction = $this->getLanguageInstruction($locale);
...
Return ONLY a JSON array of strings, nothing else. For English example: ["Add dark mode toggle", "Create contact form", "Add smooth animations"]
PROMPT;
    }
    ...
    public function getTypingPrompts(int $count = 8, ?string $locale = null): array
    {
        ...
        try {
            ...
            AiErrorAudit::report('Internal AI typing prompts failed', ['locale' => $locale], $e);
        }
...
PROMPT;
    ...
    public function getHeroHeadlines(int $count = 4, ?string $locale = null): array
    {
        ...
        try {
            ...
            AiErrorAudit::report('Internal AI hero headlines failed', ['locale' => $locale], $e);
        }
...
Return ONLY a JSON array of strings, nothing else. Example:
["Describe it. AI builds it.", "What will you create?", "Chat your website into existence", "Just describe. We build."]
PROMPT;
    }
    ...
    public function getHeroSubtitles(int $count = 4, ?string $locale = null): array
    {
        ...
        try {
            ...
            AiErrorAudit::report('Internal AI hero subtitles failed', ['locale' => $locale], $e);
        }
