        ...
        try {
            ...
            $query->chunk(100, function ($users) use ($dryRun, &$totalProcessed, &$totalReset, &$totalSkipped) {
                foreach ($users as $user) {
                    ...
                    $overageBalance = max(0, (int) ($user->build_credit_overage_balance ?? 0));
                    $newCredits = $plan->getMonthlyBuildCredits() + $overageBalance;
                    ...
                    $this->line("  Reset user {$user->id} ({$user->email}): {$oldCredits} -> {$newCredits} (overage: {$overageBalance})");
                    $totalReset++;
                }
            });
