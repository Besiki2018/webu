...
class Project extends Model
{
    ...
    protected function casts(): array
    {
        return [
            ...
            'custom_domain_ssl_attempts' => 'integer',
            'custom_domain_ssl_next_retry_at' => 'datetime',
    ...
    public function sqlExports(): HasMany
    {
        return $this->hasMany(ProjectSqlExport::class);
    }

    public function tenantDatabaseBinding(): HasOne
    {
        return $this->hasOne(TenantDatabaseBinding::class);
    }
     ...
     */
    public function appendToHistory(string $role, string $content, ?string $category = null, ?int $thinkingDuration = null): void
    {
    ...
    /**
     * Append multiple messages to conversation history in a single update (e.g. after AI site editor flow).
     *
     * @param  array<int, array{role: string, content: string, category?: string|null, thinking_duration?: int|null}>  $entries
     */
    public function appendMessages(array $entries): void
    {
        if ($entries === []) {
            return;
        }
        ...
        $history = $this->conversation_history ?? [];
        $estimatedTokens = (int) ($this->estimated_tokens ?? 0);
        ...
        foreach ($entries as $entry) {
            ...
            if (isset($entry['category']) && $entry['category'] !== null) {
                $item['category'] = $entry['category'];
            }
            if (isset($entry['thinking_duration']) && $entry['thinking_duration'] !== null && $role === 'assistant') {
                $item['thinking_duration'] = (int) $entry['thinking_duration'];
            }
            $history[] = $item;
            $estimatedTokens += (int) ceil(strlen($content) / 4);
        }
    ...
    public function getHistoryForBuilderOptimized(): array
    {
        ...
        // Prepend language rule so AI responds in the same language the user writes in (ChatGPT-like)
        $languageRule = 'Always respond in the same language the user writes in. '
            . 'If the user writes in Georgian, respond in Georgian; if in English, in English; and so on. '
            . 'Match the user\'s language in every reply, like ChatGPT. '
            . 'Keep replies natural, polished, and user-facing. '
            . 'Do not expose tool calls, internal reasoning, or developer meta commentary unless the user explicitly asks for them.';
    ...
    public function generateVerificationToken(): string
    {
        ...
        $this->update(['domain_verification_token' => $token]);
