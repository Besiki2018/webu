...
class AiProvider extends Model
{
    ...
    const DEFAULT_BASE_URLS = [
        ...
        self::TYPE_CLAUDE => 'https://api.anthropic.com',
     ...
     * For Anthropic and Claude, falls back to ANTHROPIC_API_KEY env when not set in provider.
     */
    public function getApiKey(): ?string
    {
        ...
        if (in_array($this->type, [self::TYPE_ANTHROPIC, self::TYPE_CLAUDE], true)) {
            $envKey = config('services.anthropic.api_key') ?: env('ANTHROPIC_API_KEY');

            return $envKey !== null && $envKey !== '' ? trim((string) $envKey) : null;
        }
    ...
    public function getDefaultModel(): string
    {
        ...
        return $models[0] ?? match ($this->type) {
            ...
            self::TYPE_CLAUDE => 'claude-sonnet-4-6',
    ...
    /**
     * Resolve token limit parameter name for OpenAI-compatible chat/completions.
     *
     * GPT-5 family requires max_completion_tokens instead of max_tokens.
     */
    public static function resolveTokenLimitParameter(string $providerType, string $model): string
    {
        if ($providerType !== self::TYPE_OPENAI) {
            return 'max_tokens';
        }
    ...
    /**
     * Model used only for connection test (stable, current).
     */
    protected const ANTHROPIC_TEST_MODEL = 'claude-sonnet-4-6';

    /**
     * Test Anthropic API connection.
     * Uses a fixed current model so the test validates the API key regardless of provider default.
     */
    protected function testAnthropicConnection(): array
    {
        ...
        $response = Http::withHeaders([
            ...
            'model' => self::ANTHROPIC_TEST_MODEL,
            'max_tokens' => 1,
        ...
        if (! is_string($message) && is_string($response->body())) {
            $message = substr($response->body(), 0, 300);
        }
        $message = is_string($message) ? trim($message) : 'Connection failed';
     ...
     * For Anthropic/Claude, considers ANTHROPIC_API_KEY env fallback.
     */
