    ...
    protected function loadBroadcastConfig(): void
    {
        try {
            ...
            if ($driver === 'reverb' && ! empty($settings['reverb_key'])) {
                ...
                // Broadcasting client config (how Laravel connects to Reverb)
                ...
                config(['broadcasting.connections.reverb.options.host' => $host]);
                config(['broadcasting.connections.reverb.options.port' => $port]);
                config(['broadcasting.connections.reverb.options.scheme' => $scheme]);
                config(['broadcasting.connections.reverb.options.useTLS' => $scheme === 'https']);

                // Reverb server binding config (where the server listens)
                config(['reverb.servers.reverb.port' => $port]);
                config(['reverb.servers.reverb.hostname' => $host]);
